...
res_dir = '../results_benchmark2'

A = torch.tensor([[1, 0.1],[0.5, -0.5]], dtype=type)
B = torch.tensor([[1], [0.5]], dtype=type)
C = torch.diag(torch.sqrt(torch.tensor([1, 1], dtype=type)))
...
print('number of states {}, number of inputs {}'.format(N_x, N_u))

rho = 0.1
gamma = 0.95 #discount factor
std_dev = 0.001#np.sqrt(0.1)

LQR = dlqr(A, B, C, rho, gamma, sigma=std_dev)

#state-action relevance weights, see http://www.mit.edu/~pucci/discountedLP.pdf
XU_weights = torch.cat((torch.ones(N_x, dtype=type), 0.1*torch.ones(N_u, dtype=type)), 0)
...
if args.exp_sampling:
    ...
    #exploration policy
    M_0 = M_opt + 0.02*torch.ones((N_u, N_x), dtype=type)
...
if args.LP_approach == 1:
    try:
        print('optimality gap {}, optimal o.f. {}, solver time [s] {}'.format(np.abs(E_Q-E_Qstar), E_Qstar, time))
    except:
        print('ops! looks like your problem is unbounded! solver time [s] {}'.format(time))
elif args.LP_approach == 2:
    try:
        print('optimality gap {}, gap {}, optimal o.f. {}, solver time [s] {}'.format(np.abs(E_Q-gap-E_Qstar), gap, E_Qstar, time))
    except:
        print('ops! looks like your problem is unbounded! solver time [s] {}'.format(time))
...
if args.save:
    ...
    if args.exp_sampling:
        ...
        f_name = '{}_states_{}_constraint_{}_sigma_{}_samples_{}_xrange_{}_urange_{}_symmetric_{}_run.json'.format(N_x, args.n_constraints, std_dev, args.samples, args.x_range, args.u_range, args.symmetric, args.random_seed)

    else:

        f_name = '{}_states_{}_constraint_{}_sigma_{}_samples_{}_xrange_{}_eps_{}_symmetric_{}_run.json'.format(N_x, args.n_constraints, std_dev, args.samples, args.x_range, args.eps, args.symmetric, args.random_seed)
