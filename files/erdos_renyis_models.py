...
for n_x in N_x:
    ...
    while rank_OM<n_x or rank_CM<n_x:#we require observability and controllability
        ...
        for ii in range(n_x):
            for jj in range(n_x):
                if ii!=jj:
                    A[ii, jj] = np.random.choice([0,1], p=[1-p_A, p_A])*np.random.uniform(-range_u, range_u)

        B = np.zeros((n_x, N_u))

        for ii in range(n_x):
            for jj in range(N_u):
                B[ii,jj] = np.random.choice([0,1], p=[1-p_B, p_B])*np.random.uniform(-range_b, range_b)
        ...
        rank_CM = np.linalg.matrix_rank(controllability_matrix)
        rank_OM = np.linalg.matrix_rank(observability_matrix)
